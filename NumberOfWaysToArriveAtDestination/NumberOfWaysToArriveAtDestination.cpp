// NumberOfWaysToArriveAtDestination.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
class Solution {
public:
    unordered_map<int, vector<tuple<int, int>>> buildEdges(std::vector<std::vector<int>>& roads)
    {
        unordered_map<int, vector<tuple<int, int>>> edges;
        for (const auto& road : roads) {
            auto a = road[0];
            auto b = road[1];
            auto t = road[2];
            edges[a].push_back(make_tuple(b, t));
            edges[b].push_back(make_tuple(a, t));
        }
        return edges;
    }
    ...
    int countPaths(int n, vector<vector<int>>& roads) {
        ...
        auto edges = buildEdges(roads);
        constexpr auto BigValue = numeric_limits<long long>::max();
        vector<vector<long long>> dp(n, { 0, BigValue });
        /*auto comparator = [](tuple<int, int>& t1, tuple<int, int>& t2) {
            return get<0>(t1) > get<0>(t2);
        };
        priority_queue<tuple<int, int>, vector<tuple<int, int>>, decltype(comparator)> to_visit;*/
        // priority_queue<tuple<int, int>, vector<tuple<int, int>>, greater> to_visit;
        priority_queue<tuple<long long, long long>, vector<tuple<long long, long long>>, greater<tuple<long long, long long>>> to_visit;
        // priority_queue<tuple<int, int>> to_visit;
        to_visit.push(make_tuple(0, 0));
        dp[0] = { 1, 0 };
        while (!to_visit.empty()) {
            const auto [t, current] = to_visit.top(); to_visit.pop();
            if (dp[current][1] < t) {
                continue;
            }
            for (auto& [node, time] : edges[current]) {
                long long new_time = t + time;
                if (dp[node][1] > new_time) {
                    dp[node][0] = dp[current][0];
                    dp[node][1] = new_time;
                    to_visit.push(make_tuple(new_time, node));
                }
                else if (dp[node][1] == new_time) {
                    dp[node][0] += dp[current][0];
                    dp[node][0] %= MOD;
                }
            }
...
{
    {
        ...
        cout << Solution().countPaths(200, roads) << endl; // 1
    }
    {
    ...
    }
    {
        ...
        cout << Solution().countPaths(2, roads) << endl; // 1
    }
}

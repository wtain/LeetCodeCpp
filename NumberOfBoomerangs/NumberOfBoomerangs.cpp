// NumberOfBoomerangs.cpp : Defines the entry point for the console application.
//
...
public:
	...
	*/
	...
	int numberOfBoomerangs(vector<vector<int>>& points) {
		...
		vector<unordered_map<int, int>> distcounts;
		...
		int cnt = 0;
		for (int i = 0; i < n; ++i) {
			for (const auto& dc : distcounts[i]) {
				if (dc.second < 2)
					continue;
				//cnt += 2 * C(dc.second, 2);
				cnt += (dc.second-1) * dc.second;
			}
		}
		...
		for (int i = 0; i < n; ++i) {
			for (int j = i + 1; j < n; ++j) {
				for (int k = j + 1; k < n; ++k) {
					if (dists2(i, j) == dists2(j, k))
						++cnt;
					if (dists2(i, j) == dists2(i, k))
						++cnt;
					if (dists2(k, j) == dists2(i, k))
						++cnt;
				}
			}
		}
